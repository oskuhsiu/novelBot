...
1. 從 `{{PROJECT_DIR}}/config/narrative_progress.yaml` 讀取：
   - 當前章節的 `beats` 列表

2. 從 `{{PROJECT_DIR}}/config/novel_config.yaml` 讀取：
   - `content_weights`
...
- **前置**：`structure/chapter_beater`（需要節拍列表）
- **讀取**：`memory/lorekeeper`（獲取伏筆列表）
- **輸出到**：`execution/scene_writer`（提供增強建議）
