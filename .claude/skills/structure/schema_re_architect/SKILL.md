...
- `target_characters`: 目標專案的 `角色資料庫(SQLite, via char_query.py)`
- `target_factions`: 目標專案的勢力資料庫（SQLite, via `faction_query.py`）
- `target_world`: 目標專案的世界地圖資料庫（SQLite, via `atlas_query.py`）
...
- 角色資料庫(SQLite, via char_query.py) → 可用角色
- 勢力資料庫(SQLite, via faction_query.py) → 勢力結構
- 世界地圖資料庫(SQLite, via atlas_query.py) → 場景地圖
