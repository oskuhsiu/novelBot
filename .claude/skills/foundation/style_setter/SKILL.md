...
從 `{{PROJECT_DIR}}/config/novel_config.yaml` 讀取以下欄位：
...
請輸出以下內容：
1. 風格特徵總結（200字，含旁白人格、口語密度、允許的語助詞、內心獨白形式）
2. 適合此風格的句式結構
3. 修辭手法偏好
4. 節奏感描述
5. 對話風格（正式/口語/混合，允許的廢話程度）
```
...
### Step 4: 撰寫示範段落
```
請以 {{reference_authors}} 的風格，撰寫一段 300 字的示範段落。

情境：根據類型 {{genre}} 選擇最能展示風格的場景。
（例：搞笑→荒謬日常、戀愛→拌嘴、冒險→探索、恐怖→不安暗示）
...
- **被呼叫於**：nvGenesis 的第一步
- **輸出被使用於**：
  - `execution/scene_writer`：遵循風格寫作
  - `execution/dialogue_director`：對話風格參考
  - `execution/sensory_amplifier`：描寫風格參考
