---
name: world_builder
description: 世界觀編織者 - 構建故事舞台的地理、規則與社會體系，寫入 SQLite 資料庫
---
...
此 Skill 負責構建故事發生的舞台，包含地理環境、物理法則、社會體系。它會讀取風格設定，產生完整的世界觀資料並寫入 SQLite 資料庫。
...
使用 CLI 將每個區域寫入：
```bash
.venv/bin/python tools/atlas_query.py --proj {proj} add --json '{"id":"REG_001","name":"...","region_type":"...","summary":"...","description":"...","climate":"...","tags":[...],"locations":[...]}'
```
zones 和 transit_network 也各自寫入：
```bash
.venv/bin/python tools/atlas_query.py --proj {proj} add --json '{"id":"ZONE_001","name":"...","region_type":"Dungeon","parent_id":"REG_001","summary":"...","hazard_level":3,...}'
.venv/bin/python tools/atlas_query.py --proj {proj} add --json '{"id":"TRANSIT_001","name":"...","region_type":"transit","summary":"...","stops":[...],"schedule":"...","cost":"..."}'
```
...
- **前置 Skill**：`foundation/style_setter`（需要風格指南）
- **後續 Skill**：
  - `foundation/faction_forge`：在地點上建立勢力
  - `execution/scene_writer`：使用地點資料寫場景
- **協作 Skill**：
  - `execution/chaos_engine`：可在現有地點生成特殊事件點
...
```
劇情需要主角前往一個「荒廢的研究設施」。
請根據現有的世界地圖資料庫，在 {{parent_region}} 下新增此地點。
