...
#### Step 3d: 生成動機地圖與關係動態
讀取 `execution/motivation_engine/SKILL.md` 並遵循其指令分析出場角色動機。
讀取 `memory/relationship_dynamics/SKILL.md` 並遵循其指令更新關係積分。
...
計算字數（單次呼叫批次計算所有章節）：`cd {{REPO_ROOT}} && .venv/bin/python tools/word_count.py {{PROJECT_DIR}}/output/chapters/chapter_{start}.md {{PROJECT_DIR}}/output/chapters/chapter_{start+1}.md ...`
