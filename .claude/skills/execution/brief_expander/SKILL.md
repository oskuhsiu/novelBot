...
3. **連貫性上下文**（從 ChromaDB `chapters` collection 和 `story_outline.yaml`）：
   - 上一章 `ending_summary`：使用 `ChapterVector.get_chapter(chapter_id)` 讀取
   - 當前 SubArc 摘要（story_outline）
   - 角色當前狀態
...
- **前置 Skill**：
  - `skill_brief_extractor`：可提供標準化的 brief 輸入
  - `skill_weight_balancer`：提供內容比例分配
- **協作 Skill**：
  - `skill_scene_writer`：可選擇性地呼叫以處理個別場景
  - `skill_dialogue_director`：優化對話密集場景
  - `skill_sensory_amplifier`：強化環境描寫
  - `skill_identity_validator`：驗證角色一致性
- **後續 Skill**：
  - `skill_logic_auditor`：檢查邏輯一致性
  - `skill_consistency_validator`：驗證與前後文的連貫
