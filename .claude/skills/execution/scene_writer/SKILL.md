...
3. **角色一致但活的**：對白和行為須符合 base_profile，但 traits 是傾向而非劇本。用 presence 建立角色的物理存在，用行動和對話展示性格，不用旁白宣告
4. **權重遵循**：內容比例符合 content_weights

## 輸入

1. **節拍資訊**（從 narrative_progress.yaml 的章節資料）：
   - `summary`：本節拍要寫什麼
   - `location`：場景地點
   - `characters`：出場角色
   - `action`：發生的事件
...
3. **角色資料**（從角色資料庫 SQLite，使用 `char_query.py --proj {proj} get-public {CHAR_IDS}`）

4. **內容分配**（從 weight_balancer 計算結果）

5. **連貫性上下文**（從 nvChapter Step 1.5 傳入，SubArc 資料來自 `config/outline/arc_{current_arc}.yaml`）：
...
讀取角色存在感：{{characters_presence}}（場景中的物理存在方式）
讀取存在感調變：從 memory/motivation_map.yaml 取出場角色的 presence_modulation（若存在）
  → 場景內依角色當下情緒選用對應的 presence 表現，情緒轉變時 presence 跟著變
  → 若 motivation_map.yaml 不存在或無該角色資料，fallback 到 DB 靜態 presence
```
...
8. 角色**首次出場**用 presence + appearance（物理形象），不用性格標籤或旁白介紹。同章後續場景角色已建立，可跳過入場
9. emotional_state 調節角色表現：同一角色在不同情緒下行為模式不同
...
- **前置**：
  - `structure/pacing_calculator`：決定詳細程度
  - `structure/weight_balancer`：決定內容比例
  - `structure/beat_optimizer`：提供強化建議
- **協作**：
  - `execution/dialogue_director`：優化對話
  - `execution/sensory_amplifier`：強化感官
- **後續**：
  - `memory/logic_auditor`：檢查邏輯衝突
