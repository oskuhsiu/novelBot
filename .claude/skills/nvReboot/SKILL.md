...
讀取 `{{REPO_ROOT}}/.claude/skills/foundation/world_builder/SKILL.md` 並遵循其指令生成新世界觀，寫入 `{NEW_DIR}/config/world_atlas.yaml`。
...
寫入 `{NEW_DIR}/config/character_db.yaml`，然後執行遷移寫入 SQLite：
```bash
cd {{REPO_ROOT}} && .venv/bin/python tools/migrate_db.py --proj {NEW_ALIAS} char
```

## Step 9: 重新建立勢力

讀取 `{{REPO_ROOT}}/.claude/skills/foundation/faction_forge/SKILL.md` 並遵循其指令創建 2-4 個勢力，寫入 `{NEW_DIR}/config/faction_registry.yaml`。
...
讀取 `{{REPO_ROOT}}/.claude/skills/foundation/item_smith/SKILL.md` 並遵循其指令為主角生成初始裝備，寫入 `{NEW_DIR}/config/item_compendium.yaml`，然後遷移至 SQLite：
```bash
cd {{REPO_ROOT}} && .venv/bin/python tools/migrate_db.py --proj {NEW_ALIAS} item
```
...
創建 `{NEW_DIR}/memory/` 下的檔案：
- emotion_log.yaml（空白初始化） — 情感記錄實際使用 SQLite
- narrative_progress.yaml（初始化）
