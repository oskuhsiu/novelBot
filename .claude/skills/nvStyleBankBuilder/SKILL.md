...
> [!NOTE]
> 本 skill 是 MEMORY.md「所有 sub-agent foreground」的刻意例外。原因：builder 屬長跑型任務（多站抓取、驗證、入庫），BG 可讓主 context 繼續接下一個指令，且本 skill 是 MANUAL ONLY，不在自動工作流程中觸發。
...
查已有作家：
```bash
cd {{REPO_ROOT}} && .venv/bin/python tools/style_bank_query.py list-authors
```
