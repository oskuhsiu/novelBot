...
1. 從 `config/character_db.yaml` 讀取：
   - `base_profile.core_desire`：核心慾望
   - `base_profile.fear`：核心恐懼
   - `current_state`：當前狀態

2. 從 `config/narrative_progress.yaml` 讀取：
   - 當前 Arc/SubArc 的情境
   - 最近章節的 `ending_summary`

3. 從 `memory/lore_bank.yaml` 讀取：
   - 最近發生的重大事件
   - 角色間的關係變動
