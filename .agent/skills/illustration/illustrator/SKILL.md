...
可以接受以下類型的輸入：
1. **角色 ID**：從角色資料庫讀取外貌描述（`char_query.py --proj {proj} get-base {CHAR_ID}`）
2. **場景 ID**：從世界地圖資料庫讀取環境描述（`atlas_query.py --proj {proj} get {REG_ID}`）
3. **物品 ID**：從物品資料庫讀取物品細節（`item_query.py --proj {proj} get {ITEM_ID}`）
4. **自定義描述**：使用者的直接描述
