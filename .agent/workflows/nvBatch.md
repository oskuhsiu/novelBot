...
- **若為 True（已完成所有目標章節）**：
  1. 執行最終維護（如果 parameters.maint 設為 end 或是 full）：`/nvMaint proj={proj} mode=full`
  2. 執行結束審查（如果 parameters.review 設為 light 或 end）：`/nvReview proj={proj} range={start_chapter}-{current_target-1} mode={review}`
...
#### 3.5: 額外維護 (依參數)
- 如果 `parameters.maint == "every"`，執行：`/nvMaint proj={proj} mode=full`
(註：nvChapter 已內建 light maint。若 `parameters.maint` 設為 `every`，則每章完成後都會執行 full maint；若設為 `end`，則每一章只會進行 light maint，直到整個批次結束時才在 Step 2 執行一次滿版的 full maint。)
